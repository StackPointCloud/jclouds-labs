...
public class ServerToNodeMetadata implements Function<Server, NodeMetadata> {
   ...
   @Inject
   public ServerToNodeMetadata(Function<org.apache.jclouds.profitbricks.rest.domain.Volume, Volume> fnVolume,
      ...
      if (server.dataCenterId() != null)
         location = Iterables.find(locationSupply.get(), new Predicate<Location>() {
            ...
            public boolean apply(Location t) {
               return t.getId().equals(server.dataCenterId());
            }
         });
...
//              .hostname(server.href())
              ...
              .hardware(hardware)
              .operatingSystem(mapOsType(server.properties().bootVolume().properties().licenceType()))
              .location(location)
