...
public class ServerApiMockTest extends BaseOneAndOneApiMockTest {
   ...
   public void testCreateServer() throws InterruptedException {
      ...
      assertNotNull(response);
      assertEquals(server.getRequestCount(), 1);
      assertSent(server, "POST", "/servers", "{\"name\":\"My server\",\"description\":\"My server description\",\"server_type\":\"cloud\",\"hardware\":{\"vcore\":2.0,\"cores_per_processor\":2.0,\"ram\":2.0,\"hdds\":[{\"size\":50.0,\"is_main\":true}]},\"appliance_id\":\"applianceId\",\"datacenter_id\":\"datacenterId\",\"password\":\"Test123!\",\"power_on\":true}"
      );
   }
   ...
   public void testCreateFixedInstanceServer() throws InterruptedException {
      ...
      Server response = serverApi().createFixedInstanceServer(Server.CreateFixedInstanceServer.create(
              "name", "name", ServerType.CLOUD, hardware, "applianceId", "datacenterId", "password",
